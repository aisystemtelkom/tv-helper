...
| the packet's running order of judul, as one list | **susunan judul** | the list on Berkas Order under the upload section and above the lembar periksa, drawn as soon as an order is open and not after the reading pass, because a judul for a berkas marked **tanpa AI** is added here by hand with no pass at all: one row per judul this order prints, in the order the DOKUMEN VALIDASI prints them. It is a second view of the judul list and took no control away, so every judul in the review keeps its own **Naikkan** and **Turunkan**. A usulan is never in it; it joins once a person has accepted it. Each row's handle is its packet position, so the sentence over the list names the number rather than a grip glyph |
...
| the comparison did not look in a berkas marked tanpa AI | said ONCE, at the head of the register, BY NAME: *"Pencocokan ini tidak mencari di berkas yang Anda tandai tanpa AI: ..."* | the rows keep **tidak ditemukan** and the fence keeps **tanpa AI**; neither word is bent to carry this. The berkas are the ones recorded when the comparison ran (`ConfigCheck.fenced`), never the fence as it stands, so the sentence cannot vanish while the verdicts it explains stay |
| compare again after a skipped berkas is let back in | **Cocokkan ulang dengan dokumen** | offered only when a berkas the register was compared without is now **dibaca AI**. NOT **Cari sekali lagi**: it asks the first-pass question over every isian, keeps every decision, and spends nothing of the one re-search |
